...
# How many history items actually render an inline preview. The rest still
# list (title/subtitle) but skip the <Image>/<Video> node -- keeps the panel
# payload small and fast even with a long history.
_MAX_PREVIEWS = 12
...
async def _history_section(ctx) -> ui.UINode:
    ...
    for i, doc in enumerate(docs):
        ...
        # Only the first _MAX_PREVIEWS items render a heavy <Image>/<Video>
        # node -- inlining every preview (or, worse, base64-embedding bytes)
        # bloats the /call payload and makes the panel spin forever. The rest
        # still appear as titled cards, just without the visual.
        show_preview = i < _MAX_PREVIEWS and bool(url)
        if show_preview and kind == "image":
            preview = ui.Image(src=url, alt=prompt, width="100%", caption=prompt)
        elif show_preview and kind == "video":
